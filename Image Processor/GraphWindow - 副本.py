        ...
        if self.operating_mode_2 == 0:
            ...
            for img in data.target:
                if method == 0:     # Guassian
                    ...
                    smooth_img = self.func2D.smooth_sts_mapping(img, val, area)
                    result.append(smooth_img)
            ...
            for img in data.target:
                ...
                start_column = int(ret[1][1][0][0])
                end_column = int(1 + ret[1][1][0][-1])
                # !!! need to be part = filter(ret[0], area, degree)
                if method == 0:  # Guassian
                    ...
                    # part = np.array(eng.symmetry(matlab.double(ret[0].tolist()), order))
                    part = self.func2D.symmetry(ret[0], order)
                    data.name = 'symmetry-part-' + str(order) + '-' + name
                elif method == 5:   # Delete bad point
                    # part = np.array(eng.smooth_sts_mapping(matlab.double(ret[0].tolist()), val, area))
                    part = self.func2D.smooth_sts_mapping(ret[0], val, area)
    ...
    def adv_conv(self, kernel):
        ...
        if self.operating_mode_2 == 0:
            ...
            for img in data.target:
                ...
                start_column = int(ret[1][1][0][0])
                end_column = int(1 + ret[1][1][0][-1])
                part = convolve(ret[0], kernel) / np.sum(np.array(kernel))
                if self.part_type == 1:
                    for coordinate in boundary:
                        part[coordinate[0]][coordinate[1]] = copy.deepcopy(tmp[coordinate[0]+start_row][coordinate[1]+start_column])
                for row in range(start_row, end_row):
    ...
    def get_current_parent(self):
        if self.current_list == self.listWidget:
            ...
            if name[0] == '\t':
                ...
                pname = name
            data = copy.deepcopy(self.data[self.plot_list.index(pname)].data_)
            """ ccpp1.3 """
            # data = data.swapaxes(0, 2)
            energy = copy.deepcopy(self.data[self.plot_list.index(pname)].energy)
            rscale = copy.deepcopy(self.data[self.plot_list.index(pname)].scale)
    ...
    def get_current_child(self):
        if self.current_list == self.listWidget and self.listWidget.currentItem() != None:
            ...
            if name[0] == '\t':
                ...
                pname = name
            index = self.scrollBar_Energy.value()
            # print(pname, self.data[self.plot_list.index(pname)].data_.shape)
            data = copy.deepcopy(self.data[self.plot_list.index(pname)].data_[index,:,:])

            energy = self.data[self.plot_list.index(pname)].energy
            rscale = self.data[self.plot_list.index(pname)].scale
            return data, name, energy, rscale
        elif self.current_list == self.listWidget_Processed:
            index = self.scrollBar_Energy.value()
            name = self.processed_data[self.processed_list.index(self.listWidget_Processed.currentItem().text())].name
    ...
    def roi_cut(self):
        ...
        for img in data.target:
            ...
            if self.part_type == 0:
                ...
                ret = self.part_c.getArrayRegion(tmp, self.img_display, axes=(0, 1), returnMappedCoords=True)
                boundary = self.get_circle_boundary(ret[0])
            # repalce the circle boundary with minimum value

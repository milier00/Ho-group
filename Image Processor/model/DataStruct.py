...
class SpcData(STMData):
    ...
    def _cnv_bit2volt(self, bit_data):
        ch = self.ramp_ch    # Determine ramp channel
        bias_flag = '20' if self.bias_dac else 'd'          # Conversion flag
        if ch == 0:     # Bias
            volt_data = [cnv.bv(data, bias_flag, self.dacrange[13]) for data in bit_data]
        else:           # Z offset fine
            volt_data = [cnv.bv(data, 'd', self.dacrange[2]) for data in bit_data]
        return volt_data

    def get_cnv_data(self):
        for pt in range(self.data.shape[0]):
            for ch_num in range(self.data[pt].shape[0]):
                self.data[pt][ch_num] = self._cnv_bit2volt(self.data[pt][ch_num])

                # print(np.max(self.data[pt][ch_num]), np.min(self.data[pt][ch_num]))

...
class MouseDragHandler(object):
    ...
    def mouseDragEvent(self, ev):
        ...
        if ev.isStart():
            if ev.button() == QtCore.Qt.LeftButton:
                ...
                if self.dragMode is not None:
                    ...
                    # print("move!")              # added by dan 3/8/2021
                    # print(self.startPos)        # added by dan 3/8/2021
                    ev.accept()
                else:
                    ev.ignore()
            else:
        ...
        if ev.isFinish() and self.dragMode is not None:
            roi._moveFinished()
            # print("finish!")                # added by dan 3/8/2021
            return
        ...
        if self.dragMode == 'translate':
            ...
            # print(newPos)                   # added by dan 3/8/2021
...
class EllipseROI(ROI):
    ...
    **Arguments**
    ...
    def getArrayRegion_(self, arr, img=None, axes=(0, 1), **kwds):
        """
    ...
    def getArrayRegion(self, arr, img=None, axes=(0, 1), returnMappedCoords=False, **kwds):
        ...
        ## generate an ellipsoidal mask
        mask = np.fromfunction(lambda x, y: np.hypot(((x + 0.5) / (w / 2.) - 1), ((y + 0.5) / (h / 2.) - 1)) < 1,
                               (w, h))
